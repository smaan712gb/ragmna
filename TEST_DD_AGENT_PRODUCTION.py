"""
Comprehensive DD Agent Test - Production Version
Tests production-ready DD Agent with enhanced error handling
"""
...
DD_AGENT_URL = "http://localhost:8017"  # Production DD Agent port
...
def test_dd_agent_health():
    ...
    print("\n" + "="*80)
    print("TEST 1: DD Agent Health Check (Production)")
    print("="*80)
    
    try:
        response = requests.get(f"{DD_AGENT_URL}/health", timeout=10)
        result = response.json()
        print(f"✓ DD Agent is healthy: {result}")
        print(f"\n  Service Version: {result.get('version', 'unknown')}")
        print(f"  Connected Services:")
        for service, status in result.get('connected_services', {}).items():
...
def test_comprehensive_dd_analysis(symbol: str):
    ...
    print("\n" + "="*80)
    print(f"TEST 2: Comprehensive DD Analysis (Production) for {symbol}")
    print("="*80)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n🏭 Production Ready: {result.get('production_ready', False)}")
            ...
            if 'vector_sources' in result:
                ...
                print(f"  - Sources integrated: {[s for s in vector_sources.get('sources_integrated', []) if s]}")
...
def main():
    ...
    print("\n" + "="*80)
    print("DD AGENT PRODUCTION TEST SUITE")
    print("Testing Production-Ready DD Agent with Enhanced Error Handling")
    print("="*80)
    
    test_results = {
        'test_suite': 'DD Agent Production Test',
    ...
    if dd_result:
        ...
        # Check 1: Production flag
        validation_checks.append(('Production Ready Flag', dd_result.get('production_ready', False)))
        
        # Check 2: Ingestion Vectors (OK if fallback works)
        vector_sources = dd_result.get('vector_sources', {})
        validation_checks.append(('Vector Sources Configured', 'sources_integrated' in vector_sources))
        
        # Check 3: RAG vectors available (should be True via company_data)
        rag_categories = vector_sources.get('rag_vectors_by_category', {})
        all_rag_available = all(rag_categories.values())
        validation_checks.append(('RAG Vectors Available', all_rag_available))
        
        # Check 4: RAG contexts retrieved
        total_rag = vector_sources.get('total_rag_contexts', 0)
        validation_checks.append(('RAG Contexts Retrieved', total_rag > 0))
        
        # Check 5: All risk categories analyzed
        risk_categories_present = all(
            cat in dd_result for cat in 
            ['legal_analysis', 'financial_analysis', 'operational_analysis', 
             'strategic_analysis', 'reputational_analysis']
        )
        validation_checks.append(('All Risk Categories Analyzed', risk_categories_present))
        
        # Check 6: RAG insights present
        ...
        # Check 7: Financial model analysis
        model_analysis_present = 'financial_model_analysis' in dd_result
        validation_checks.append(('Financial Model Analysis Present', model_analysis_present))
        
        # Check 8: Document insights
        ...
        if all_passed:
            ...
            print("✅ DD Agent is PRODUCTION READY")
            print("✅ Can handle missing services gracefully")
            print("✅ Provides RAG insights using company data")
    ...
    # Save results
    output_file = save_test_results(test_results, 'DD_AGENT_PRODUCTION_TEST')
